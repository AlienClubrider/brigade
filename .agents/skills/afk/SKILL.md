...
tradeoff **consented** and **explicit**: the head chef is stepping away, so the
sub-supervisor may triage routine wakes in bash instead of waking brigade's
LLM for each one. Escalations still reach the head chef, but as one pre-read,
...
1. **Set the durable away-mode flag:**
   ...
   date '+%s' > state/.afk
   ```
   This file survives a brigade restart: recovery re-enters afk if the
   flag is present.
...
2. **Ensure the sub-supervisor daemon is running.** Check the pid file; start
   ...
   if [ -f state/.supervise-daemon.pid ] && kill -0 "$(cat state/.supervise-daemon.pid)" 2>/dev/null; then
     ...
     nohup bin/brigade-supervise-daemon.sh >/dev/null 2>&1 &
   fi
   ```
   The daemon is **presence-gated**: it injects escalations only while
   `state/.afk` exists, and stays quiet otherwise.

3. **Do not separately arm `brigade-watch.sh`.** The daemon manages the watcher as
   its child; the singleton lock no-ops a stray arm harmlessly.

4. **Acknowledge** to the head chef that away-mode is active: the daemon will
   self-handle routine wakes, escalate only head chef-relevant events, and the
   head chef can exit by sending any real message.
...
- A message **without** the sentinel marker and **not** starting with `/afk`
  -> the head chef is back. Clear `state/.afk`, stop the daemon, flush one
  ...
  responsiveness (arm `bin/brigade-watch-arm.sh`).
...
Bias ambiguous cases toward exit: a present head chef beats token savings, and
a false exit is self-correcting (the head chef re-runs `/afk`).

## Orthogonal to approval authority

afk changes how aggressively brigade surfaces things, **not who approves
what**. "Away" never means "approves more." A PR ready for merge, a
needs-decision finding, or anything destructive still waits for the head chef's
...
separator, 0x1f), invisible and untypable. This is how brigade tells a
...
injection (shared with `brigade-send.sh` via `bin/brigade-zellij-lib.sh`):
...
protects against the race window between the head chef returning and their
...
`brigade-send.sh` uses the same primitive and exits non-zero
when a steer's Enter is positively swallowed, so brigade learns an instruction
did not land instead of leaving it unsubmitted.

## Classification policy

The daemon wraps `brigade-watch.sh`, runs the watcher as a child, classifies each
wake reason in bash, and self-handles the routine majority without consuming a
brigade turn.
Only head chef-relevant events escalate to brigade's context, and even then as
one pre-read, single-line, batched digest.

Classify each wake this way:

- `signal` whose status content has no head chef-relevant verb
  (`done:|needs-decision:|blocked:|failed:|PR ready|checks green|ready in branch|merged`)
  -> self-handle. Head chef-relevant verb -> escalate.
- `check` -> always escalate. Check scripts print only when brigade should wake.
- `stale` with a terminal status -> escalate. Non-terminal stale is transient:
  ...
  tick: a delay, never a loss. Healthy line cooks are autonomous and do not wait
  on brigade mid-ticket.
- `heartbeat` -> self-handle. The daemon runs its own cheap bash fleet scan
  ...
  head chef-relevant status line the per-wake classifier might miss.
...
the marker lets brigade distinguish it from a real head chef message.
...
- **Composer guard on the supervisor pane** - before injecting, the daemon
  ...
  its digest into the head chef's half-typed line.
- The composer detector, shared with `brigade-send.sh` in `bin/brigade-zellij-lib.sh`, drops
...
- **Marker strip** - `strip_injection_marker` removes the sentinel prefix before
  classification or relay, so the digest text brigade sees is clean.
- **Portable singleton lock** - the daemon uses the repo's portable lock helper
  (`brigade-wake-lib.sh`) instead of `flock`, which is absent on macOS.
...
- **Auto-discovered supervisor pane** - the daemon resolves its injection target
  from `FM_SUPERVISOR_TARGET`, then `$TMUX_PANE`, then a `brigade:0` fallback
...
- Nothing is lost. The durable queue plus `brigade-wake-drain.sh` recover any missed
