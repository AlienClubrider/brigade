...
You talk to a single agent - the brigade - and it runs the kitchen for you: spawning autonomous agents in WezTerm tabs, giving each a clean git worktree, supervising them to completion, and handing you finished PRs, approved local merges, or standalone investigation reports.
...
- **A visible kitchen** - every line cook works in its own WezTerm tab you can watch or type into; the brigade reconciles.
...
- **Restart-proof** - all state lives on disk and in WezTerm; kill the session anytime and the next one reconciles and carries on.

Full detail on every feature lives in [docs/architecture.md](docs/architecture.md).

## Quick Start

**Requirements:** WezTerm, a verified agent harness (claude, codex, opencode, or pi), git with GitHub auth, and worktrunk (`brew install worktrunk`).
...
Then open WezTerm, navigate to brigade, and launch your harness:

```sh
cd brigade && claude   # or codex, opencode, pi — AGENTS.md takes over
```

Add the tab-color snippet from `docs/wezterm-lua.md` to your WezTerm config so your tab bar shows fleet state in color.
...
# brigade clones the project under projects/ and spawns two line cooks in WezTerm tabs
...
Every line cook window is a WezTerm tab in your own session — watch the kitchen work in real time or type into any tab to intervene.
...
```
 ...
 └──┬──────────────┬───────────────┬───┘
    │ wezterm cli send-text / status files │
    ▼              ▼               ▼
 ┌────────┐   ┌────────┐      ┌────────┐
 │brigade-ticket1│   │brigade-ticket2│  ... │brigade-ticketN│   WezTerm tabs ⏳🔴✅
...
It routes each request to a line cook in its own WezTerm tab and git worktree, supervises the fleet with a zero-token event-driven watcher, and brings you finished PRs, approved local merges, or investigation reports.
...
- [docs/expeditor.md](docs/expeditor.md) - WezTerm tab colors and optional notifications (the Expeditor).
- [docs/wezterm-lua.md](docs/wezterm-lua.md) - WezTerm Lua config snippet for tab coloring.
