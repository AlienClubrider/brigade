#!/usr/bin/env bash
# Hand already-identified, in-scope backlog items off from the main brigade
# backlog to a sous-chef's own home backlog. Use this when a sous-chef is
# created (or whenever an existing queued item should become its domain's work)
# so the sous-chef owns its queue from day one instead of the item staying
# stranded in the main backlog.
#
# Scope-matching is brigade's JUDGMENT: you pass the ticket-id keys you have
# already judged in-scope for the sous-chef. This script performs only the
# mechanical move - it removes each matched line from data/backlog.md under the
# active brigade home and appends it, under the same section heading, to the
# sous-chef home's data/backlog.md (home resolved from data/sous-chefs.md). It
# never changes a line's text, never writes into a project (it refuses a home
# that is not a brigade home), and is idempotent: a key already present in the
# sous-chef backlog is reported and skipped, so re-running converges. If any key
# matches neither backlog, nothing is moved. See AGENTS.md project management
# and ticket lifecycle.
# Usage: brigade-backlog-handoff.sh <sous-chef-id> <item-key>...
set -eu
...
[ $# -ge 2 ] || { echo "usage: brigade-backlog-handoff.sh <sous-chef-id> <item-key>..." >&2; exit 1; }
ID=$1
shift

sous-chef_home() {
  local id=$1 line
  [ -f "$REG" ] || { echo "error: no sous-chef registry at $REG" >&2; return 1; }
  line=$(grep -E "^- $id( |$)" "$REG" | tail -1 || true)
  [ -n "$line" ] || { echo "error: sous-chef $id is not registered in $REG" >&2; return 1; }
...
validate_sous-chef_home() {
  ...
  if [ ! -f "$abs_home/.brigade-sous-chef-home" ]; then
    echo "error: brigade home $home is not a seeded sous-chef home" >&2
    return 1
  fi
  marker_id=$(cat "$abs_home/.brigade-sous-chef-home" 2>/dev/null || true)
...
RAW_HOME=$(sous-chef_home "$ID") || exit 1
[ -n "$RAW_HOME" ] || { echo "error: sous-chef $ID has no home in $REG" >&2; exit 1; }
SUB_HOME=$(validate_sous-chef_home "$ID" "$RAW_HOME") || exit 1
SUB_BACKLOG="$SUB_HOME/data/backlog.md"
validate_backlog_file "main backlog" "$MAIN_BACKLOG" || exit 1
validate_backlog_file "sous-chef backlog" "$SUB_BACKLOG" || exit 1
...
KEYS_FILE=$(mktemp "$MAIN_DIR/.brigade-handoff-keys.XXXXXX")
MOVED_FILE=$(mktemp "$MAIN_DIR/.brigade-handoff-moved.XXXXXX")
KEPT_FILE=$(mktemp "$MAIN_DIR/.brigade-handoff-kept.XXXXXX")
SUB_TMP=$(mktemp "$SUB_DIR/.brigade-handoff-sub.XXXXXX")
MAIN_BAK=$(mktemp "$MAIN_DIR/.brigade-handoff-main-bak.XXXXXX")
SUB_BAK=$(mktemp "$SUB_DIR/.brigade-handoff-sub-bak.XXXXXX")
CHANGES_STARTED=0
COMMITTED=0
cleanup() {
