...
# Usage: brigade-fleet-sync.sh [<project-dir>]
set -eu
...
usage() {
  echo "usage: brigade-fleet-sync.sh [<project-dir>]" >&2
}
...
prune_gone_branches() {
  ...
  # that still has a worktree (a live or not-yet-torn-down ticket). "Gone" plus
...
sync_project() {
  ...
  mode_line=$("$FM_ROOT/bin/brigade-project-mode.sh" "$label" 2>/dev/null || echo "no-mistakes off")
  mode=${mode_line%% *}
