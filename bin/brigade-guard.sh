...
# supervision scripts.
# First, always warn if the brigade primary checkout (FM_ROOT) is on a named
# non-default branch, because that means brigade-on-itself work landed in the
# primary instead of an isolated worktree.
# Then, if any ticket is in flight (a state/<id>.meta exists) and the watcher's
...
. "$SCRIPT_DIR/brigade-wake-lib.sh"
# shellcheck source=bin/brigade-tangle-lib.sh
. "$SCRIPT_DIR/brigade-tangle-lib.sh"

# Worktree-tangle alarm, checked FIRST and independent of in-flight tickets: the
# brigade PRIMARY checkout (FM_ROOT) must stay on its default branch. If a
# line cook's branch/commits landed here instead of in its own isolated worktree,
...
# only: detached HEAD (linked worktrees, sous-chef homes) never trips this.
...
if [ -n "$tangle_branch" ]; then
  ...
  trule='━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'
  {
    ...
    printf '●  A line cook likely branched/committed in the primary instead of its own worktree.\n'
...
# Portable mtime; see brigade-watch.sh for why the `stat -f || stat -c` fallback breaks on Linux.
...
# Only act with tickets in flight; count them so the banner can say how much is
...
# No fresh watcher with tickets in flight is the dangerous state: emit a prominent,
# bordered banner FIRST so it reads as an alarm, not a buried stderr line.
if [ "$watcher_fresh" = false ]; then
  if "$queue_pending"; then
    fix='After draining queued wakes, re-arm the watcher: run bin/brigade-watch-arm.sh as the harness-tracked background task (never a shell & that gets reaped).'
  else
    fix='Re-arm it NOW: run bin/brigade-watch-arm.sh as the harness-tracked background task (never a shell & that gets reaped).'
  fi
  rule='━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'
  {
    printf '●%s\n' "$rule"
...
if "$queue_pending"; then
  echo "WARNING: queued wakes pending - drain them with bin/brigade-wake-drain.sh before anything else." >&2
fi
exit 0
