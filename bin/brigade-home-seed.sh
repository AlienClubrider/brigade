#!/usr/bin/env bash
# Provision and route persistent sous-chef homes.
#
# Usage:
...
#       initialized, a .brigade-sous-chef-home marker is written, and
#       data/sous-chefs.md is updated.
#       Seeding is transactional: on validation, clone, init, or registry failure,
#       generated briefs, new homes, new project clones, and registry edits are
#       rolled back. Worktrunk-acquired homes are returned only when the rollback
...
usage() {
  echo "usage: brigade-home-seed.sh <id> <home|-> <project>..." >&2
  echo "       brigade-home-seed.sh validate" >&2
}
...
validate_registry_home_text() {
  ...
  case "$home" in
    *';'*|*')'*|*$'\n'*)
      echo "error: sous-chef home path contains registry delimiters: $home" >&2
      return 1
      ;;
  esac
}
...
validate_registry() {
  ...
  tmp=$(mktemp "${TMPDIR:-/tmp}/brigade-brigades.XXXXXX")
  if [ -f "$REG" ]; then
    while IFS= read -r line; do
      case "$line" in
        "- "*)
  ...
  ' "$tmp" 2>/dev/null) || {
    rm -f "$tmp"
    printf 'error: duplicate sous-chef home assignment:\n%s\n' "$duplicate_homes" >&2
    return 1
  }
  duplicate_ids=$(awk -F '\t' '
    {
  ...
  ' "$tmp" 2>/dev/null) || {
    rm -f "$tmp"
    printf 'error: duplicate sous-chef id assignment:\n%s\n' "$duplicate_ids" >&2
    return 1
  }
  overlaps=$(awk -F '\t' '
  ...
  ' "$tmp" 2>/dev/null) || {
    rm -f "$tmp"
    printf 'error: overlapping sous-chef home assignment:\n%s\n' "$overlaps" >&2
    return 1
  }
  rm -f "$tmp"
  return 0
...
validate_seed_leaf_files() {
  ...
  for label in "data/projects.md" "data/charter.md" "$SUB_HOME_MARKER"; do
    path="$home/$label"
    if [ -L "$path" ]; then
      echo "error: sous-chef leaf file must not be a symlink: $path" >&2
      return 1
    fi
    [ -e "$path" ] || continue
    ...
    case "$abs_path" in
      "$abs_home"/*) ;;
      *)
        echo "error: sous-chef leaf file must resolve inside the sous-chef home: $path" >&2
        return 1
        ;;
    esac
  done
...
validate_project_destination() {
  ...
  if ! path_is_ancestor_of "$abs_home" "$abs_projects"; then
    echo "error: sous-chef projects directory must resolve inside the sous-chef home: $projects_dir" >&2
    return 1
  fi
  if ! path_is_ancestor_of "$abs_projects" "$abs_dst"; then
    echo "error: seeded project $project destination must resolve inside the sous-chef projects directory: $dst" >&2
    return 1
  fi
  if [ "$abs_dst" = "$abs_active_home" ] || path_is_ancestor_of "$abs_active_home" "$abs_dst"; then
    echo "error: seeded project $project destination cannot be inside the active brigade home: $dst" >&2
    return 1
  fi
  if [ "$abs_dst" = "$abs_root" ] || path_is_ancestor_of "$abs_root" "$abs_dst"; then
    echo "error: seeded project $project destination cannot be inside the brigade repo: $dst" >&2
    return 1
  fi
  printf '%s\n' "$abs_dst"
}
...
ensure_home() {
  ...
  if [ "$requested" = "-" ]; then
    home=$(acquire_worktrunk_home "$id")
    verify_brigade_home "$home"
    return
  fi
...
verify_brigade_home() {
  local home=$1
  refuse_active_home_path "$home" || return 1
  [ -f "$home/AGENTS.md" ] || { echo "error: $home is not a brigade home (missing AGENTS.md)" >&2; return 1; }
  [ -d "$home/bin" ] || { echo "error: $home is not a brigade home (missing bin/)" >&2; return 1; }
...
validate_home_assignment() {
  ...
  if [ -f "$home/$SUB_HOME_MARKER" ]; then
    ...
    if [ "$marker_id" != "$id" ]; then
      echo "error: sous-chef home $home is already marked for ${marker_id:-unknown}" >&2
      return 1
    fi
  fi
  id_conflict=$(registry_id_conflict_for_assignment "$id" "$home" || true)
  if [ -n "$id_conflict" ]; then
    echo "error: sous-chef id $id is already registered to home $id_conflict; retire it before assigning $home" >&2
    return 1
  fi
...
EOF
  if [ "$conflict_type" = exact ]; then
    echo "error: sous-chef home $home is already registered to $owner" >&2
    return 1
  fi
  echo "error: sous-chef home $home overlaps registered sous-chef home $registered_home for $owner" >&2
  return 1
}
...
clone_project() {
  ...
  read -r mode _ <<EOF
$(FM_HOME="$FM_HOME" FM_DATA_OVERRIDE="$DATA" "$FM_ROOT/bin/brigade-project-mode.sh" "$project")
EOF
  if [ "$mode" = local-only ]; then
    echo "error: project $project is local-only; sous-chef routes support only no-mistakes and direct-PR projects" >&2
    return 1
  fi
  if [ -e "$dst" ]; then
...
seed_rollback_target() {
  ...
  if [ "$abs_target" = "$abs_home" ]; then
    echo "REFUSED: unsafe $label rollback target $target is the active brigade home" >&2
    return 1
  fi
  if [ "$abs_target" = "$abs_root" ]; then
    echo "REFUSED: unsafe $label rollback target $target is the brigade repo" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_target" "$abs_home"; then
    echo "REFUSED: unsafe $label rollback target $target is an ancestor of the active brigade home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_target" "$abs_root"; then
    echo "REFUSED: unsafe $label rollback target $target is an ancestor of the brigade repo" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_home" "$abs_target"; then
    echo "REFUSED: unsafe $label rollback target $target is inside the active brigade home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_root" "$abs_target"; then
    echo "REFUSED: unsafe $label rollback target $target is inside the brigade repo" >&2
    return 1
  fi
  printf '%s\n' "$abs_target"
}
...
seed_project_rollback_target() {
  ...
  if ! path_is_ancestor_of "$abs_home" "$abs_projects"; then
    echo "REFUSED: unsafe created project rollback target $target has projects directory outside the sous-chef home" >&2
    return 1
  fi
  if ! path_is_ancestor_of "$abs_projects" "$abs_target"; then
    echo "REFUSED: unsafe created project rollback target $target is outside the sous-chef projects directory" >&2
    return 1
  fi
  printf '%s\n' "$abs_target"
}
...
seed_rollback() {
  ...
  if [ -n "${SEED_HOME:-}" ] && [ "$SEED_HOME" != "/" ]; then
    if [ "$SEED_HOME_ACQUIRED" = 1 ]; then
      seed_return_worktrunk_home "$SEED_HOME"
  ...
  if [ -n "${SEED_BACKUP_DIR:-}" ]; then
    restore_seed_file "$SEED_PARENT_REG_EXISTED" "$SEED_BACKUP_DIR/parent-sous-chefs.md" "$REG"
...
project_mode_in_home() {
  ...
  read -r mode _ <<EOF
$(FM_ROOT_OVERRIDE='' FM_STATE_OVERRIDE='' FM_DATA_OVERRIDE='' FM_PROJECTS_OVERRIDE='' FM_CONFIG_OVERRIDE='' FM_HOME="$home" "$FM_ROOT/bin/brigade-project-mode.sh" "$project")
EOF
  printf '%s\n' "$mode"
}
...
seed_home() {
  ...
  shift 2
  [ $# -gt 0 ] || { echo "error: sous-chef needs at least one project" >&2; return 1; }
  ...
  SEED_HOME_ACQUIRED=0
  SEED_HOME_BACKED_UP=0
  SEED_BACKUP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/brigade-home-seed.XXXXXX")
  ...
  if [ -f "$REG" ]; then
    ...
    cp "$REG" "$SEED_BACKUP_DIR/parent-sous-chefs.md"
  fi
  ...
  if [ ! -f "$SEED_PARENT_BRIEF" ]; then
    [ -n "${FM_SECONDMATE_CHARTER:-}" ] || {
      echo "error: no filled sous-chef charter brief at $SEED_PARENT_BRIEF; set FM_SECONDMATE_CHARTER or scaffold one and replace {TASK}" >&2
      return 1
    }
    [ -d "$DATA/$id" ] || SEED_PARENT_BRIEF_DIR_CREATED=1
    "$FM_ROOT/bin/brigade-brief.sh" "$id" --sous-chef "$@"
    SEED_PARENT_BRIEF_CREATED=1
  fi
  if grep -F '{TASK}' "$SEED_PARENT_BRIEF" >/dev/null 2>&1; then
    echo "error: sous-chef charter brief at $SEED_PARENT_BRIEF still contains {TASK}; fill it before seeding" >&2
    return 1
  fi
  charter_summary=$(registry_summary_for_brief "$SEED_PARENT_BRIEF")
  [ -n "$charter_summary" ] || {
    echo "error: sous-chef charter brief at $SEED_PARENT_BRIEF has an empty Charter section; fill it before seeding" >&2
    return 1
  }
  charter_scope=$(registry_scope_for_brief "$SEED_PARENT_BRIEF")
  [ -n "$charter_scope" ] || {
    echo "error: sous-chef charter brief at $SEED_PARENT_BRIEF has an empty Routing scope section; fill it before seeding" >&2
    return 1
  }
