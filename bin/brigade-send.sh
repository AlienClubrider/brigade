#!/usr/bin/env bash
# Send one line of literal text to a line cook window, then Enter.
# Usage: brigade-send.sh <window> <text...>
...
# Special keys instead of text: brigade-send.sh <window> --key Escape   (or Enter, C-c, ...)
#
...
# all retries), brigade-send exits NON-ZERO so the caller knows the steer did not land
...
# bin/brigade-zellij-lib.sh. Tune with FM_SEND_RETRIES (default 3) / FM_SEND_SLEEP (0.4).
set -eu
