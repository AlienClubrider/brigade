#!/usr/bin/env bash
# brigade-supervise-daemon.sh — presence-gated sub-supervisor (closes #27's P2).
#
# Wraps bin/brigade-watch.sh: runs it as a child, classifies each wake reason, and
# either SELF-HANDLES the routine majority in bash (no brigade turn) or
# ESCALATES a batched, distilled digest to the supervisor pane on
# head chef-relevant events only. This is the token-efficient replacement for the
# prior always-inject daemon: routine signal/stale/heartbeat wakes cost zero
# brigade context; only done/needs-decision/blocked/failed/persistent-wedge/
...
# user message clears it and brigade resumes full per-wake responsiveness.
# When afk is off, the daemon stays quiet — it self-handles routine wakes and
# buffers escalations without injecting, so the base one-shot brigade-watch.sh
...
# at the start of a message. Brigade's contract: a message that starts with
# the marker is an internal escalation (stay afk); a message without it means
# the head chef is back (exit afk, flush catch-up, resume per-wake responsiveness).
...
#     injection is recovered on the next brigade-wake-drain.sh. The daemon does not
...
#     line cook is therefore detected within STALE_ESCALATE_SECS + a tick, never
#     lost. Line cooks are autonomous, so a delayed stale response does not stall
#     a healthy line cook's own progress.
...
#     state/*.status for a head chef-relevant line the per-wake classifier might
#     have missed (e.g. a status verb outside HEAD CHEF_RE) and escalates it.
#
...
#
# Usage: brigade-supervise-daemon.sh
...
#                                   head chef-relevant escalation for matching
#                                   kinds.
...
#          FM_BUSY_REGEX            OR-ed busy signatures (mirrors brigade-watch.sh)
...
#                                   structural and style-aware (bin/brigade-zellij-lib.sh):
...
#          lock. A crashing brigade-watch.sh is logged and restarted, never killing
...
# injection is prefixed with this byte; brigade treats a leading marker as an
# internal escalation (stay afk) and its absence as "head chef is back" (exit afk).
...
# --- portable stat (same trap as brigade-watch.sh: no `stat -f || stat -c`) -------
...
# skill (enter) and by brigade on user return (exit). Durable: a plain file,
...
# should_exit_afk: encodes brigade's afk-exit contract as a testable function.
...
#   anything else           -> 0 (head chef is back; exit afk)
# Bias toward exit: only the marker and an explicit /afk invocation keep afk
# alive. A false exit is self-correcting (the head chef re-runs /afk).
...
# marker (a daemon escalation), 1 otherwise (a real user message). Brigade's
# afk-exit contract uses this: marker present -> stay afk; absent -> head chef is
...
# distilled content brigade relays to the head chef or feeds back to classifiers.
...
# summary brigade would otherwise have to re-read.
...
classify_check() {  # <full reason>  — check scripts print only when brigade should wake
  printf 'escalate|%s' "$1"
}
...
# Seen:     state/.subsuper-seen-status-<ticket>  last status line the scan
...
# Record the seen-status marker for a head chef-relevant status line so the
# heartbeat catch-all scan does not re-fire it. The single source of truth for
# the .subsuper-seen-status-<ticket> dedup state: called from both the per-wake
...
# Busy + composer-empty detection are the shared primitives in brigade-zellij-lib.sh
# (one source of truth with brigade-send.sh). These thin wrappers keep the daemon's
...
# an ERROR, drops a durable marker brigade/recovery can surface, and flashes
...
#     head chef-relevant line the per-wake classifier missed and escalate it.
housekeeping() {  # <state>
  ...
  for marker in "$state"/.subsuper-stale-*; do
    ...
    # so probe the live brigade-* windows for one whose ticket matches).
    win=$(window_for_task "$key" 2>/dev/null || true)
    if [ -z "$win" ]; then
      # Window gone (ticket torn down): drop the marker, nothing to escalate.
      rm -f "$marker"; continue
    fi
    if pane_is_busy "$win"; then
      rm -f "$marker"   # line cook resumed: benign
    else
  ...
  # (3) heartbeat scan (catch-all for a head chef-relevant status the per-wake
...
inject_msg() {  # <message> [state]
  ...
  # daemon self-handles and stays quiet; brigade drives the base one-shot
  ...
  # them. Then prepend the sentinel marker — brigade's afk-exit contract
...
trim_log() {
  ...
  tmp=$(mktemp "${TMPDIR:-/tmp}/brigade-daemon-log.XXXXXX") || return 0
...
# classifiers above are sourceable for unit tests (tests/brigade-daemon.test.sh).
# ============================================================================
...
fm_super_main() {
  ...
  # shellcheck source=bin/brigade-wake-lib.sh
  FM_STATE_OVERRIDE="$STATE" . "$FM_DAEMON_DIR/brigade-wake-lib.sh"

  local WATCH="$FM_DAEMON_DIR/brigade-watch.sh"
  ...
  if ! fm_lock_try_acquire "$LOCK"; then
    if [ -n "${FM_LOCK_HELD_PID:-}" ]; then
      echo "error: another brigade-supervise-daemon is already running (pid $FM_LOCK_HELD_PID, lock $LOCK held)" >&2
    else
      echo "error: another brigade-supervise-daemon is already running (lock $LOCK held)" >&2
    fi
    exit 1
  fi
  echo "$$" > "$PIDFILE"
  ...
  start_watcher() {
    CUR_TMP=$(mktemp "${TMPDIR:-/tmp}/brigade-watch.XXXXXX") || { log "error: mktemp failed; retrying in 5s"; sleep 5; return 1; }
