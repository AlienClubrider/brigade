...
# Shared worktree-tangle guard for the brigade-on-itself case.
# Usage: . bin/brigade-tangle-lib.sh
#
# Brigade is a worktrunk-pooled git repo of itself: line cook worktrees and
# sous-chef homes are all linked `git worktree`s of the same repo, while the
# PRIMARY checkout (the repo root brigade operates from) is a normal checkout
# on a real branch - normally the default branch, main. The "worktree tangle"
# failure mode is a line cook spawned to work on brigade ITSELF branching and
...
# which is how every linked worktree and sous-chef home legitimately sits on the
...
# echo nothing and return 1. Detached HEAD is how linked worktrees and sous-chef
