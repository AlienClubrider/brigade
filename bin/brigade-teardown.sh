...
# Scout tickets (kind=scout in meta) carve out of that check: their worktree is
# declared scratch and the report at data/<ticket-id>/report.md is the work
...
SECONDMATE_REG="$DATA/sous-chefs.md"
...
backlog_refresh_reminder() {
  ...
  if fm_tasks_axi_compatible; then
    ...
    printf '%s\n' "Backlog: $ID just finished. Run $done_cmd, then run tickets-axi ready for dependency-cleared candidates, check date gates, and dispatch only work whose blockers are gone and date is due."
  else
...
brigade_home_has_worktrunk_slot() {
  local home=$1
...
validate_removal_target() {
  ...
  if [ -n "$abs_home" ] && [ "$abs_target" = "$abs_home" ]; then
    echo "REFUSED: unsafe $label removal target $target is the active brigade home" >&2
    return 1
  fi
  if [ "$abs_target" = "$abs_root" ]; then
    echo "REFUSED: unsafe $label removal target $target is the brigade repo" >&2
    return 1
  fi
  if [ -n "$abs_home" ] && path_is_ancestor_of "$abs_target" "$abs_home"; then
    echo "REFUSED: unsafe $label removal target $target is an ancestor of the active brigade home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_target" "$abs_root"; then
    echo "REFUSED: unsafe $label removal target $target is an ancestor of the brigade repo" >&2
    return 1
  fi
  if [ -n "$abs_home" ] && path_is_ancestor_of "$abs_home" "$abs_target"; then
    echo "REFUSED: unsafe $label removal target $target is inside the active brigade home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_root" "$abs_target"; then
    echo "REFUSED: unsafe $label removal target $target is inside the brigade repo" >&2
    return 1
  fi
  printf '%s\n' "$abs_target"
}
...
validate_brigade_operational_dirs_for_removal() {
  ...
  for name in data state config projects; do
    ...
    if [ -L "$dir" ] && [ ! -e "$dir" ]; then
      echo "REFUSED: unsafe $label $name directory $dir resolves outside the sous-chef home" >&2
      return 1
    fi
    if [ -d "$dir" ]; then
...
validate_child_worktree_for_removal() {
  ...
  if abs_home=$(cd "$FM_HOME" 2>/dev/null && pwd -P); then
    if path_is_ancestor_of "$abs_home" "$abs_target"; then
      echo "REFUSED: unsafe child worktree removal target $target is inside the active brigade home" >&2
      return 1
    fi
  fi
  abs_root=$(cd "$FM_ROOT" && pwd -P)
  if path_is_ancestor_of "$abs_root" "$abs_target"; then
    echo "REFUSED: unsafe child worktree removal target $target is inside the brigade repo" >&2
    return 1
  fi
...
validate_brigade_home_for_removal() {
  ...
  if [ ! -f "$abs_home_path/$SUB_HOME_MARKER" ]; then
    echo "REFUSED: unsafe $label removal target $home is not a seeded sous-chef home" >&2
    return 1
  fi
  if [ -n "$expected_id" ]; then
    marker_id=$(cat "$abs_home_path/$SUB_HOME_MARKER" 2>/dev/null || true)
    if [ "$marker_id" != "$expected_id" ]; then
      echo "REFUSED: unsafe $label removal target $home is marked for sous-chef ${marker_id:-unknown}, expected $expected_id" >&2
      return 1
    fi
  fi
  validate_brigade_operational_dirs_for_removal "$abs_home_path" "$label" || return 1
  conflict=$(registered_descendant_home_for_removal "$SECONDMATE_REG" "$abs_home_path" || true)
  if [ -z "$conflict" ]; then
    conflict=$(registered_descendant_home_for_removal "$abs_home_path/data/sous-chefs.md" "$abs_home_path" || true)
  fi
  if [ -n "$conflict" ]; then
    IFS=$'\t' read -r child_id child_home <<EOF
$conflict
EOF
    echo "REFUSED: unsafe $label removal target $home contains registered sous-chef home $child_home for $child_id" >&2
    return 1
  fi
  printf '%s\n' "$abs_home_path"
}
...
validate_brigade_home_children_removal() {
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    if [ "$child_kind" = sous-chef ]; then
      ...
      validate_brigade_home_for_removal "$child_home" "child brigade home" "$child_id" >/dev/null || return 1
      validate_brigade_home_children_removal "$child_home" || return 1
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    if [ "$child_kind" = sous-chef ]; then
      ...
      if [ -n "$child_home" ] && [ -d "$child_home" ]; then
        cleanup_brigade_home_children "$child_home"
        remove_brigade_home "$child_home" "child brigade home" "$child_id"
      fi
    elif [ -n "$child_wt" ] && [ -d "$child_wt" ]; then
      validate_child_worktree_for_removal "$child_wt" "$child_proj" >/dev/null || return 1
...
remove_sous-chef_registry_entry() {
  local id=$1 tmp
...
if [ "$KIND" = sous-chef ]; then
  ...
  validate_brigade_home_for_removal "$HOME_PATH" "sous-chef home" "$ID" >/dev/null || exit 1
  if [ "$FORCE" = "--force" ]; then
    validate_brigade_home_children_removal "$HOME_PATH" || exit 1
  fi
fi

if [ "$KIND" = sous-chef ] && [ "$FORCE" != "--force" ]; then
  SUB_STATE="$HOME_PATH/state"
  if [ -d "$SUB_STATE" ]; then
    for child_meta in "$SUB_STATE"/*.meta; do
      [ -e "$child_meta" ] || continue
      echo "REFUSED: sous-chef $ID still has in-flight work in $SUB_STATE." >&2
...
if [ "$KIND" = sous-chef ] && [ "$FORCE" = "--force" ]; then
  cleanup_brigade_home_children "$HOME_PATH"
fi
...
if [ -d "$WT" ] && [ "$FORCE" != "--force" ]; then
  if [ "$KIND" = sous-chef ]; then
    ...
    if [ ! -f "$REPORT" ]; then
      echo "REFUSED: scout ticket $ID has no report at $REPORT." >&2
      echo "The report is the work product. Have the line cook write it (or get the head chef's explicit OK to discard, then --force)." >&2
      exit 1
    fi
  else
    # The brigade-spawn hook file is ours, never work product; ignore it in the dirty check.
    ...
    if [ -n "$unpushed" ] && [ "$MODE" = local-only ]; then
      ...
      # into the local default branch (brigade does that merge on the head chef's
      ...
      if [ -n "$dirty" ] || [ -n "$unmerged" ]; then
        ...
        echo "Merge the branch into local $DEFAULT first (bin/brigade-merge-local.sh after the head chef approves), or push to a fork/remote, or get the head chef's explicit OK to discard, then --force." >&2
        exit 1
      fi
      ...
      echo "Push the branch (or get the head chef's explicit OK to discard, then --force)." >&2
      exit 1
    fi
  fi
fi

# Best-effort: drop the local ticket branch so the shared repo does not accumulate refs.
if [ -d "$WT" ] && [ "$KIND" != sous-chef ]; then
...
if [ "$KIND" = sous-chef ]; then
  ...
  remove_sous-chef_registry_entry "$ID"
fi
