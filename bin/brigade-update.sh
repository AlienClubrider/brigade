#!/usr/bin/env bash
# Self-update a running brigade and its sous-chefs to the latest origin.
#
# Mechanical half of the /updatebrigade skill. Fast-forwards the running
# brigade repo's default branch from origin, then fast-forwards every
# registered sous-chef home (each a worktrunk worktree of this same repo, or
# a standalone clone) the same way. FAST-FORWARD ONLY, exactly like
# brigade-fleet-sync.sh: never force, never create a merge commit, never stash;
...
# dirs (data/, state/, config/, projects/, .no-mistakes/), so a sous-chef's
...
# fetched on their own. Sous-Chef homes are leased at a detached HEAD on the
# default branch, so a fast-forward there advances HEAD only and never touches
# any other worktree's checkout or the shared `main` branch.
#
# It does NOT re-read AGENTS.md or nudge sous-chefs itself - those are LLM /
# tmux actions the skill performs. The script's job is the safe git mechanics
# plus a parseable summary telling the caller what to do next:
#   - one status line per target (updated/already current/skipped)
#   - reread-brigade: yes|no    (did the running brigade's instructions change)
#   - nudge-sous-chefs: <window-targets...>|none   (updated live sous-chefs to nudge)
#
# Usage: brigade-update.sh [--help]
set -eu
...
SECONDMATES_MD="$FM_HOME/data/sous-chefs.md"
SUB_HOME_MARKER=".brigade-sous-chef-home"

"$SCRIPT_DIR/brigade-guard.sh" || true

usage() { echo "usage: brigade-update.sh [--help]" >&2; }
...
validate_operational_dirs() {
  ...
  for name in data state config projects; do
    dir="$abs_home/$name"
    if [ -L "$dir" ] && [ ! -e "$dir" ]; then
      VALIDATION_ERROR="sous-chef $name directory must resolve inside the sous-chef home"
      return 1
    fi
    if [ -d "$dir" ]; then
      abs_dir=$(cd "$dir" && pwd -P) || {
        VALIDATION_ERROR="sous-chef $name directory cannot be resolved"
        return 1
      }
    elif [ -e "$dir" ]; then
      VALIDATION_ERROR="sous-chef $name path is not a directory"
      return 1
    else
    ...
    if [ "$abs_dir" = "$abs_active_home" ] || path_is_ancestor_of "$abs_active_home" "$abs_dir"; then
      VALIDATION_ERROR="sous-chef $name directory cannot be inside the active brigade home"
      return 1
    fi
    if [ "$abs_dir" = "$abs_root" ] || path_is_ancestor_of "$abs_root" "$abs_dir"; then
      VALIDATION_ERROR="sous-chef $name directory cannot be inside the brigade repo"
      return 1
    fi
  done
}

validate_sous-chef_home() {
  ...
  abs_active_home=$(resolved_existing_dir "$FM_HOME") || {
    VALIDATION_ERROR="active brigade home is not a directory"
    return 1
  }
  abs_root=$(resolved_existing_dir "$FM_ROOT") || {
    VALIDATION_ERROR="brigade repo is not a directory"
    return 1
  }
  if [ "$abs_home" = "/" ]; then
    VALIDATION_ERROR="sous-chef home cannot be the filesystem root"
    return 1
  fi
  if [ "$abs_home" = "$abs_active_home" ]; then
    VALIDATION_ERROR="sous-chef home cannot be the active brigade home"
    return 1
  fi
  if [ "$abs_home" = "$abs_root" ]; then
    VALIDATION_ERROR="sous-chef home cannot be the brigade repo"
    return 1
  fi
  if path_is_ancestor_of "$abs_active_home" "$abs_home"; then
    VALIDATION_ERROR="sous-chef home cannot be inside the active brigade home"
    return 1
  fi
  if path_is_ancestor_of "$abs_root" "$abs_home"; then
    VALIDATION_ERROR="sous-chef home cannot be inside the brigade repo"
    return 1
  fi
  if path_is_ancestor_of "$abs_home" "$abs_active_home"; then
    VALIDATION_ERROR="sous-chef home cannot be an ancestor of the active brigade home"
    return 1
  fi
  if path_is_ancestor_of "$abs_home" "$abs_root"; then
    VALIDATION_ERROR="sous-chef home cannot be an ancestor of the brigade repo"
    return 1
  fi
  validate_operational_dirs "$abs_home" "$abs_active_home" "$abs_root" || return 1
  if [ -L "$abs_home/$SUB_HOME_MARKER" ]; then
    VALIDATION_ERROR="sous-chef marker must not be a symlink"
    return 1
  fi
  if [ ! -f "$abs_home/$SUB_HOME_MARKER" ]; then
    VALIDATION_ERROR="not a seeded sous-chef home"
    return 1
  fi
  marker_id=$(cat "$abs_home/$SUB_HOME_MARKER" 2>/dev/null || true)
  if [ "$marker_id" != "$id" ]; then
    VALIDATION_ERROR="marked for sous-chef ${marker_id:-unknown}, expected $id"
    return 1
  fi
  if [ ! -f "$abs_home/AGENTS.md" ]; then
    VALIDATION_ERROR="not a brigade home (missing AGENTS.md)"
    return 1
  fi
  if [ ! -d "$abs_home/bin" ]; then
    VALIDATION_ERROR="not a brigade home (missing bin/)"
    return 1
  fi
...
reread_brigade="no"
ff_target "$FM_ROOT" "brigade" no no
...
process_sous-chef() {
  ...
  if ! validate_sous-chef_home "$id" "$home"; then
    echo "sous-chef $id: skipped: unsafe home: $VALIDATION_ERROR"
    return 0
  fi
  ...
  ff_target "$home_real" "sous-chef $id" yes yes
  if [ "$FF_STATUS" = "updated" ] && [ -n "$window" ]; then
    nudge_windows="$nudge_windows $window"
  fi
}

# Live direct reports first: state/<id>.meta with kind=sous-chef carries the
# authoritative home= path.
if [ -d "$STATE" ]; then
  for meta in "$STATE"/*.meta; do
    [ -f "$meta" ] || continue
    grep -q '^kind=sous-chef' "$meta" 2>/dev/null || continue
    id=$(basename "$meta" .meta)
    home=$(grep '^home=' "$meta" 2>/dev/null | tail -1 | cut -d= -f2- || true)
    window=$(grep '^window=' "$meta" 2>/dev/null | tail -1 | cut -d= -f2- || true)
    process_sous-chef "$id" "$home" "$window"
  done
fi

# Registry backstop: a sous-chef registered in data/sous-chefs.md but without
...
if [ -f "$SECONDMATES_MD" ]; then
  while IFS= read -r line; do
    ...
    process_sous-chef "$id" "$home" ""
  done < "$SECONDMATES_MD"
fi

# --- caller action summary -------------------------------------------------

echo "reread-brigade: $reread_brigade"
echo "nudge-sous-chefs:${nudge_windows:- none}"
