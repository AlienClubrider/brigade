#!/usr/bin/env bash
# Safe, home-scoped (re-)arm of the brigade watcher, with honest verification.
#
# The watcher (bin/brigade-watch.sh) is one-shot: it blocks until a wake is due, prints
# one reason line, and exits. Reliability depends on re-arming through a mechanism
# that SURVIVES the call and NOTIFIES on exit, so brigade must run this script as
# the harness's own tracked background ticket (e.g. run_in_background). NEVER fire it
...
# single source of truth, shared with brigade-watch.sh and brigade-guard.sh), and prints
...
# bin/brigade-watch.sh`: that pattern matches every brigade home's watcher
# (sous-chef homes run the same script) and would kill siblings.
set -u
...
WATCH="$SCRIPT_DIR/brigade-watch.sh"
...
watch_lock_matches_pid() {
  ...
  lock_home=$(cat "$WATCH_LOCK/brigade-home" 2>/dev/null || true)
...
# harness-tracked ticket) tears the watcher down too, and the watcher's eventual
# wake exit propagates out so the harness re-notifies brigade.
child=
child_out=
cleanup_child() {
