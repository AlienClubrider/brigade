...
#   signal: <file>...     a line cook wrote a status line or a turn-end hook fired; signals
#                         landing within FM_SIGNAL_GRACE of each other coalesce into one wake
#   stale: <window>       a line cook pane stopped changing and shows no busy signature
#   check: <script>: <out> a per-ticket check script (e.g. merged-PR poll) produced output
#   heartbeat              fleet review due; starts at FM_HEARTBEAT and backs off to FM_HEARTBEAT_MAX
# For normal supervision, re-arm after each wake by running bin/brigade-watch-arm.sh
...
WATCH_PATH="$SCRIPT_DIR/brigade-watch.sh"
...
WATCHER_PID=${BASHPID:-$$}
printf '%s\n' "$FM_HOME" > "$WATCH_LOCK/brigade-home" || true
...
while :; do
  ...
  # Liveness beacon for brigade-guard.sh: a fresh mtime here means a watcher is
  # alive. Supervision scripts warn when this goes stale with tickets in flight.
  touch "$STATE/.last-watcher-beat"

  # Slow per-ticket checks (brigade writes these, e.g. a merged-PR poll).
  # Time-based via .last-check mtime so the cadence survives watcher restarts.
  # Evaluated BEFORE the signal scan: wake() exits the cycle, so a check placed
  # after the signal scan would be starved whenever a chatty sibling line cook
  ...
  # waking: a line cook's final status write and the same turn's turn-end hook
  # land seconds apart, and reporting them as separate wakes costs a full
  # brigade turn each. The re-scan also picks up a newer signature for an
...
EOF
  ...
  # signature means the line cook finished, is waiting, or is wedged. Each distinct
  ...
  # Heartbeat: brigade reviews the whole fleet at a regular cadence no matter
