#!/usr/bin/env bash
# tests/brigade-afk-inject-e2e.test.sh - private-socket end-to-end test for the afk
...
#     A head chef-relevant status must deliver exactly ONE sentinel-prefixed,
...
STATE_DIR=$(mktemp -d "${TMPDIR:-/tmp}/brigade-afk-e2e.XXXXXX")
mkdir -p "$STATE_DIR"
...
TMUX_SHIM_DIR=$(mktemp -d "${TMPDIR:-/tmp}/brigade-shim.XXXXXX")
...
# Create a fake line cook window (the watcher lists brigade-* windows for stale
# detection). The pane is an inert shell — it just needs to exist.
"$REAL_TMUX" -L "$SOCKET" new-window -d -n brigade-fake-c1 -t supervisor
...
test_scenario_a() {
  ...
  # head chef returning and starting to type before afk has been cleared.
  "$REAL_TMUX" -L "$SOCKET" send-keys -t "$SUPERVISOR_PANE" -l "human draft text"
  sleep 0.5

  # Write a head chef-relevant status to trigger a real escalation through the
  # real watcher child.
...
test_scenario_b() {
  ...
  # Write a head chef-relevant status to trigger a real escalation.
...
# No human input, no swallowed Enter: a head chef-relevant status must produce
...
test_scenario_c() {
  ...
  stop_daemon
  pass "Scenario C: a normal head chef status injects exactly one clean single-line sentinel digest"
}
