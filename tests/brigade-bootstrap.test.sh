...
# is well. brigade consumes the exact 'MISSING: worktrunk (install: ...)' and
# 'TASKS_AXI: available' lines, so those contracts are pinned verbatim. The cases
# are table-driven over the two inputs that vary: whether `worktrunk get --help`
# advertises --lease, and which (if any) tickets-axi version is on PATH.
set -u
...
# worktrunk's `get --help` advertises --lease only when FM_FAKE_TREEHOUSE_LEASE_HELP=1.
make_fake_toolchain() {
  local dir=$1 fakebin
  fakebin=$(fm_fakebin "$dir")
  fm_fake_exit0 "$fakebin" zellij node no-mistakes gh-axi chrome-devtools-axi
  cat > "$fakebin/gh" <<'SH'
#!/usr/bin/env bash
...
if [ "${1:-}" = get ] && [ "${2:-}" = --help ]; then
  if [ "${FM_FAKE_TREEHOUSE_LEASE_HELP:-}" = 1 ]; then
    printf '%s\n' 'Usage: worktrunk get [--lease] [--lease-holder <holder>]'
  else
    printf '%s\n' 'Usage: worktrunk get'
  fi
  exit 0
...
#   <label>^<lease 1/0>^<tickets-axi version or ->^<mode>^<expect>^<notcontains>
...
test_bootstrap_reporting() {
  ...
  while IFS='^' read -r label lease tasks mode expect notcontains; do
    ...
    out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$case_dir/home" FM_ROOT_OVERRIDE="$case_dir/home" \
      FM_FAKE_TREEHOUSE_LEASE_HELP="$lease" "$ROOT/bin/brigade-bootstrap.sh")
    case "$mode" in
      empty)
...
worktrunk --lease support is accepted silently^1^-^empty^^
worktrunk without --lease reports an upgrade, gh auth is fine^0^-^grep^MISSING: worktrunk (install: curl -fsSL https://kunchenguid.github.io/worktrunk/install.sh | sh)^NEEDS_GH_AUTH
...
ROWS
  pass "bootstrap reports worktrunk lease + tasks-axi compatibility contracts"
}
