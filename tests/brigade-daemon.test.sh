#!/usr/bin/env bash
# tests/brigade-daemon.test.sh - supervise-daemon classifiers, the head chef-relevant
...
# brigade-send swallow reporting, composer-pending ANSI parsing). The operator-visible
# inject flow lives in brigade-afk-inject-e2e and brigade-wake-daemon-lifecycle-e2e.
set -u

# shellcheck source=tests/wake-helpers.sh
. "$(dirname "${BASH_SOURCE[0]}")/wake-helpers.sh"

DAEMON="$ROOT/bin/brigade-supervise-daemon.sh"
...
if [ -z "${FM_TEST_DAEMON_SOURCED:-}" ]; then
  ...
  # shellcheck source=bin/brigade-supervise-daemon.sh
  . "$DAEMON"
fi

TMP_ROOT=$(fm_test_tmproot brigade-daemon-tests)


test_daemon_state_root_uses_fm_home() {
  local dir home override out
  dir=$(make_supercase daemon-brigade-home)
  home="$dir/brigade-home"
...
test_classify_terminal_signal_escalates() {
  ...
  for kw in "done: PR https://x/y/pull/1" "needs-decision: pick A" "blocked: no perms" \
    ...
    case "$out" in escalate\|*) ;; *) fail "head chef verb did not escalate ($kw): $out" ;; esac
  done
  pass "head chef-relevant status verbs escalate"
}
...
test_stale_transient_self_records_marker() {
  ...
  stale_marker_record "sess:brigade-qux-w4" "$state"
  out=$(FM_STATE_OVERRIDE="$state" classify_stale "sess:brigade-qux-w4" "$state")
  case "$out" in self\|*) ;; *) fail "transient stale did not self-handle: $out" ;; esac
  key=$(printf '%s' "$(window_to_task "sess:brigade-qux-w4")" | tr ':/.' '___')
...
test_handle_wake_routes_self_and_escalate() {
  ...
  [ -s "$state/.subsuper-escalations" ] || fail "head chef signal was not buffered by handle_wake"
  pass "handle_wake routes routine->self and head chef->escalate"
}
...
test_inject_skip_forces_self() {
  ...
  pass "INJECT_SKIP forces self-handle, bypassing head chef-relevant classification"
}
...
test_is_wake_reason_distinguishes_status_stdout() {
  ...
  is_wake_reason "stale: s:brigade-x" || fail "stale: not recognized as wake"
...
test_signal_escalate_marks_seen_no_catchall_refire() {
  ...
  [ -s "$state/.subsuper-escalations" ] || fail "head chef signal was not escalated"
  key=$(printf '%s' "sig-t8" | tr ':/.' '___')
  [ "$(cat "$state/.subsuper-seen-status-$key" 2>/dev/null || true)" = "done: PR https://x/y/pull/8" ] \
    || fail "head chef signal escalate did not write the seen-status marker"
  : > "$state/.subsuper-escalations"
  rm -f "$state/.subsuper-last-scan"
  FM_STATE_OVERRIDE="$state" housekeeping "$state"
  [ ! -s "$state/.subsuper-escalations" ] || fail "catch-all scan re-fired an already-escalated signal"
  pass "head chef signal escalate marks seen so the catch-all scan does not re-fire"
}
...
test_busy_guard_defers_when_supervisor_busy() {
  ...
  # pane shows a busy signature (brigade mid-turn)
...
test_marker_detection() {
  ...
  should_exit_afk "$state" "status update please" \
    || fail "plain message should exit afk (head chef is back)"
...
test_afk_turn_exemption() {
  ...
  # a non-/afk skill invocation DOES exit (the head chef is actively working)
...
test_pane_input_pending_bordered_with_text_is_pending() {
  ...
  # still read as pending so the daemon defers (and the head chef-return race is
  # still protected).
  local dir state fakebin capture
...
test_submit_ack_reports_pending_on_persistent_swallow() {
  ...
  # reported as "pending" — the daemon keeps the buffer, brigade-send exits non-zero —
...
test_fm_send_exits_nonzero_on_confirmed_swallow() {
  # brigade-send.sh must exit NON-ZERO when a steer's Enter is positively swallowed
  # (text left in the composer), so brigade learns the instruction did not land
  ...
  pass "brigade-send exits non-zero on a confirmed swallow, zero on a clean submit"
}
