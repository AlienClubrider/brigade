#!/usr/bin/env bash
# tests/brigade-sous-chef-lifecycle-e2e.test.sh - the happy-path sous-chef operator
...
# lease-specific paths live in brigade-sous-chef-safety.test.sh.
#
...
#   - spawn meta records kind=sous-chef, home=, and the project list; launch runs
#     in the subhome with the persistent charter and cleared operational overrides
#   - a bare `brigade-<id>` send targets the window recorded in THIS home's meta
...
TMP_ROOT=$(fm_test_tmproot brigade-sous-chef-lifecycle)
...
EOF
  ...
  FM_SECONDMATE_SCOPE='customer onboarding from brief' \
    scaffold_sous-chef_charter "$HOME_DIR" design 'customer onboarding charter' alpha beta gamma \
    || fail "filled sous-chef charter scaffold failed"
}

phase_seed() {
  local out
  out=$(PATH="$FAKEBIN:$PATH" FM_HOME="$HOME_DIR" \
    "$ROOT/bin/brigade-home-seed.sh" design "$SUB" alpha beta gamma) \
    || fail "seed failed"
  SUB_ABS=$(cd "$SUB" && pwd -P)

  assert_contains "$out" "home=$SUB_ABS" "seed did not report the subhome"
  assert_present "$SUB/.brigade-sous-chef-home" "seed did not mark the subhome"
  ...
  assert_grep '- design - customer onboarding charter' "$HOME_DIR/data/sous-chefs.md" "registry summary not from the charter"
  assert_grep 'scope: customer onboarding from brief' "$HOME_DIR/data/sous-chefs.md" "registry scope not from the filled brief"
  assert_grep 'projects: alpha, beta, gamma' "$HOME_DIR/data/sous-chefs.md" "registry did not record the project list"
  assert_no_grep 'owns:' "$HOME_DIR/data/sous-chefs.md" "registry used the legacy owns field"

  # Delivery modes preserved in the subhome registry; validation passes.
  [ "$(FM_HOME="$SUB" "$ROOT/bin/brigade-project-mode.sh" alpha)" = "direct-PR on" ] \
    || fail "alpha delivery mode not preserved in the subhome"
  [ "$(FM_HOME="$SUB" "$ROOT/bin/brigade-project-mode.sh" beta)" = "direct-PR off" ] \
    || fail "beta delivery mode not preserved in the subhome"
  FM_HOME="$HOME_DIR" "$ROOT/bin/brigade-home-seed.sh" validate >/dev/null || fail "registry validation failed after seed"
...
phase_spawn() {
  ...
  assert_grep 'kind=sous-chef' "$meta" "spawn meta did not record kind=sous-chef"
  ...
  # and never ran a project-style worktrunk get.
  assert_grep "FM_HOME='$SUB_ABS'" "$LOG" "sous-chef launch did not set FM_HOME to the subhome"
...
EOF
  local out before
  out=$(FM_HOME="$HOME_DIR" "$ROOT/bin/brigade-backlog-handoff.sh" design feat-x feat-y) \
  ...
  FM_HOME="$HOME_DIR" "$ROOT/bin/brigade-backlog-handoff.sh" design feat-x feat-y >/dev/null 2>&1 \
...
phase_teardown() {
  ...
  assert_no_grep '- design ' "$HOME_DIR/data/sous-chefs.md" "teardown did not remove the registry route"
