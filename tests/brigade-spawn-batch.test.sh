#!/usr/bin/env bash
# Behavior tests for brigade-spawn.sh batch dispatch (`id=repo` pairs).
#
# These exercise argument routing only: each spawn attempt fails fast at the
# missing-brief check, which is reached before any tmux/worktrunk side effect, so
...
SPAWN="$ROOT/bin/brigade-spawn.sh"
TMP_ROOT=$(fm_test_tmproot brigade-spawn-batch)

# Clear ambient brigade overrides so the behavior test owns its environment.
run_spawn() {
  FM_ROOT_OVERRIDE='' \
    FM_HOME='' \
    FM_STATE_OVERRIDE='' \
...
# A projects/ path is resolved through the brigade home, never the caller cwd,
...
ROWS
  pass "projects/ paths are scoped through the brigade home for single-task spawn"
}
