...
#
# Brigade is a worktrunk-pooled git repo of itself: linked worktrees and
# sous-chef homes all sit at a detached HEAD on the default branch, while the
# PRIMARY checkout (FM_ROOT) is a normal checkout on a real branch. The "tangle"
# is a line cook branching/committing in the primary instead of its own worktree,
...
#            brigade-spawn refuses to launch unless the resolved worktree is isolated.
#   GUARD 2 (detection)  - brigade-guard and brigade-bootstrap alarm when the primary is on
#            a feature branch, and stay silent on the default branch or detached.
# These cases pin: the shared lib's branch classification, the brigade-guard banner,
# the brigade-bootstrap problem line, the brief assertion ordering, and the brigade-spawn
...
. "$ROOT/bin/brigade-tangle-lib.sh"

TMP_ROOT=$(fm_test_tmproot brigade-tangle-guard)
...
detached HEAD on default is healthy (worktrees, sous-chef homes)|detached||
ROWS
...
# --- GUARD 2a: brigade-guard banner ----------------------------------------------

run_guard() {
  # Scope the guard to a temp repo as the primary checkout; state lives under it.
  FM_ROOT_OVERRIDE="$1" FM_HOME="$1" "$ROOT/bin/brigade-guard.sh" 2>&1
}
...
test_guard_banner() {
  ...
  pass "brigade-guard: bordered tangle banner fires only for a feature branch in the primary"
}

# --- GUARD 2b: brigade-bootstrap problem line ------------------------------------

run_bootstrap() {
  # No projects/ under the home keeps fleet sync inert; grep isolates the line.
  FM_ROOT_OVERRIDE="$1" FM_HOME="$1" "$ROOT/bin/brigade-bootstrap.sh" 2>/dev/null
}
...
test_bootstrap_line() {
  ...
  pass "brigade-bootstrap: TANGLE problem line fires only for a feature branch in the primary"
}

# --- GUARD 1a: brief isolation assertion ------------------------------------

# The generated ship brief must carry the isolation assertion AHEAD of the
# `git checkout -b` step, so the line cook verifies its worktree before branching.
test_brief_assertion_precedes_branch() {
  local home brief iso br
  home="$TMP_ROOT/brief-home"
  mkdir -p "$home/data"
  FM_HOME="$home" "$ROOT/bin/brigade-brief.sh" tangle-brief-cc3 alpha >/dev/null 2>&1
  ...
  pass "brigade-brief: ship brief asserts worktree isolation before the branch step"
}
...
test_spawn_isolation_abort() {
  ...
  pass "brigade-spawn: aborts unless the resolved worktree is a genuine, isolated worktree"
}
