#!/usr/bin/env bash
# Tests for bin/brigade-teardown.sh's unpushed-work safety check.
#
# Covers the local-only fork-remote fix: a local-only-registered project whose
# ticket pushes its work to a fork (upstream-contribution PRs) must be teardown-
...
TEARDOWN="$ROOT/bin/brigade-teardown.sh"
TMP_ROOT=$(fm_test_tmproot brigade-teardown-tests)

# Build a fresh sandbox for one test case. Sets up:
#   $CASE/state/        - brigade state dir (with a fresh watcher beacon)
#   $CASE/fakebin/      - mocks for worktrunk, tmux (PATH-prepended by caller)
#   $CASE/origin.git/   - bare upstream repo (so the project clone has origin)
#   $CASE/project/      - clone of origin; acts as the brigade project dir
#   $CASE/wt/           - a worktree of the project (the ticket worktree)
# Echoes the case dir.
make_case() {
...
SH
  ...
  # Add a worktree on a fresh ticket branch; that branch is where the line cook commits.
  git -C "$case_dir/project" worktree add -q -b fm/task-x1 "$case_dir/wt" main

  # Fresh watcher beacon so brigade-guard stays quiet.
...
# Write a meta file for the ticket. Args: case_dir mode kind
write_meta() {
...
# Commit something on the worktree's ticket branch. Args: case_dir [message]
wt_commit() {
...
# the worktree's ticket branch to it and fetch into the project so the worktree
...
add_fork_with_pushed_branch() {
  ...
  # Push the ticket branch from the worktree to the fork, then fetch into project
  # so refs/remotes/fork/brigade-ticket-x1 is visible from the worktree (shared object db).
...
test_no_mistakes_origin_remote_allows() {
  ...
  # Push the ticket branch to origin and fetch so the worktree sees it.
