#!/usr/bin/env bash
# tests/brigade-wake-daemon-lifecycle-e2e.test.sh - the watcher + supervise-daemon
...
#   drain queued records -> exactly ONE head chef-relevant digest is buffered
...
# brigade-watch.sh runs plus the daemon's own functions. The head chef-relevant
# status-phrase matrix and the lock-primitive races stay as focused units
# (brigade-daemon.test.sh, brigade-watcher-lock.test.sh) - an e2e cannot deterministically
...
TMP_ROOT=$(fm_test_tmproot brigade-wake-daemon-e2e)
...
test_routine_then_terminal_after_restart() {
  ...
  FM_STATE_OVERRIDE="$state" handle_wake "signal: $status_file" "$state"
  [ -s "$state/.subsuper-escalations" ] || fail "head chef-relevant terminal status was not buffered"
