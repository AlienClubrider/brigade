#!/usr/bin/env bash
# tests/brigade-wake-queue.test.sh - wake-queue losslessness (the queue safety matrix):
...
# brigade-watcher-lock.test.sh; daemon classification/injection in brigade-daemon.test.sh.
set -u
...
TMP_ROOT=$(fm_test_tmproot brigade-wake-tests)
...
test_atomic_double_drain() {
  ...
  append_wake "$state" stale 's:brigade-task' 'stale: s:brigade-task' || fail "stale append failed"
  FM_STATE_OVERRIDE="$state" "$DRAIN" > "$out1" &
  pid1=$!
