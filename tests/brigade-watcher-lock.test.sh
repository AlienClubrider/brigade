#!/usr/bin/env bash
# tests/brigade-watcher-lock.test.sh - watcher singleton + lock-primitive races +
...
WATCH="$ROOT/bin/brigade-watch.sh"
WATCH_ARM="$ROOT/bin/brigade-watch-arm.sh"
DRAIN="$ROOT/bin/brigade-wake-drain.sh"
LIB="$ROOT/bin/brigade-wake-lib.sh"

TMP_ROOT=$(fm_test_tmproot brigade-watcher-lock-tests)
...
test_guard_warnings() {
  ...
  # (1) watcher down (no beacon) + two in-flight tickets + a queued wake.
  ...
  FM_ROOT_OVERRIDE="$dir" FM_STATE_OVERRIDE="$state" FM_GUARD_GRACE=1 "$ROOT/bin/brigade-guard.sh" 2> "$err" >/dev/null || fail "guard failed"
  ...
  grep -F 'bin/brigade-watch-arm.sh' "$err" >/dev/null || fail "guard banner missing the fix command"
  ...
  FM_ROOT_OVERRIDE="$dir" FM_STATE_OVERRIDE="$state" FM_GUARD_GRACE=300 "$ROOT/bin/brigade-guard.sh" 2> "$err" >/dev/null || fail "guard failed"
...
test_watch_restart_rejects_reused_pid() {
  ...
  printf '%s\n' "$dir" > "$state/.watch.lock/brigade-home"
